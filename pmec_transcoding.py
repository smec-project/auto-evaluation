...
def test_connections():
    ...
    missing_hosts = [host for host in required_hosts if not connection_status.get(host, False)]
    
    if missing_hosts:
        logger.error(f"Error: Missing connections to required hosts: {missing_hosts}")
        return False
...
def start_experiment():
    ...
    logger.info("Starting PMEC environment setup (LTE + 5G + PMEC Controller)...")
    ...
    if not pmec_results.get('overall_success', False):
        ...
        logger.error(f"LTE restart success: {pmec_results.get('lte_restart', {}).get('success', False)}")
        logger.error(f"5G gNB start success: {pmec_results.get('5g_gnb_start', {}).get('success', False)}")
        logger.error(f"PMEC controller start success: {pmec_results.get('pmec_controller_start', {}).get('success', False)}")
        return False
    
    logger.info("✓ PMEC environment setup completed successfully")
    logger.info(f"  - LTE service restarted on amari")
    logger.info(f"  - 5G gNB started on edge0 (PID: {pmec_results.get('5g_gnb_start', {}).get('pid', 'N/A')})")
    ...
    logger.info("  - Video transcoding PMEC server (tmux: video_transcoding_pmec)")
    logger.info("✓ PMEC application clients are running on amari:")
    logger.info("  - File transfer PMEC client (tmux: file_transfer_pmec)")
    logger.info("  - Video transcoding PMEC client (tmux: video_transcoding_pmec)")
...
def end_experiment():
    ...
    if controller_results.get('overall_success', False):
        ...
        logger.warning(f"  PMEC server stop success: {controller_results.get('server', {}).get('success', False)}")
        logger.warning(f"  PMEC client stop success: {controller_results.get('client', {}).get('success', False)}")
        success = False
    ...
    if cleanup_results.get('overall_success', False):
        ...
        logger.warning(f"  LTE stop success: {cleanup_results.get('lte_stop', {}).get('success', False)}")
        logger.warning(f"  5G gNB stop success: {cleanup_results.get('5g_gnb_stop', {}).get('success', False)}")
        logger.warning(f"  PMEC controller stop success: {cleanup_results.get('pmec_controller_stop', {}).get('success', False)}")
        success = False
    ...
    if success:
        ...
        logger.info("✓ PMEC controller system stopped")
        logger.info("✓ PMEC environment (LTE + 5G + PMEC Controller) cleaned up")
...
def print_usage():
    ...
    print("    3. Start PMEC app servers on edge1 (file transfer + video transcoding)")
    print("    4. Start PMEC app clients on amari (file transfer + video transcoding)")
    print("")
    print("  End (1):")
