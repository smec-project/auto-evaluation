...
class AppClientExecutor:
    ...
    def start_file_transfer_client(self) -> Dict[str, Any]:
        ...
        try:
            result = self.host_manager.execute_on_host(
                host_name="amari",
                command=command,
                session_name="file_transfer"
            )
            
            if result['success']:
                self.logger.info("File transfer client started successfully")
                self.logger.info(f"Session name: {result.get('session_name', 'N/A')}")
            else:
                self.logger.error(f"Failed to start file transfer client: {result['error']}")
                
            return result
            
        except Exception as e:
            self.logger.error(f"Exception during file transfer client startup: {e}")
            return {
                'success': False,
                'error': str(e),
                'pid': None,
    ...
    def stop_file_transfer_client(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Exception during file transfer client cleanup: {e}")
    ...
    def start_file_transfer_pmec_client(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['success']:
                self.logger.info("File transfer PMEC client started successfully")
                self.logger.info(f"Session name: {result.get('session_name', 'N/A')}")
            else:
                self.logger.error(f"Failed to start file transfer PMEC client: {result['error']}")
                
            return result
            
        except Exception as e:
            self.logger.error(f"Exception during file transfer PMEC client startup: {e}")
            return {
                'success': False,
                'error': str(e),
                'pid': None,
    ...
    def stop_file_transfer_pmec_client(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Exception during file transfer PMEC client cleanup: {e}")
            return {'success': False, 'error': str(e)}
    ...
    def start_video_transcoding_client(self) -> Dict[str, Any]:
        ...
        command = (
            ...
            "python3 run_amarisoft.py ~/video/Inter4K-255-slice16-20M-pingpong-loop8.mp4 2"
        )
        ...
        try:
            ...
            if result['success']:
                self.logger.info("Video transcoding client started successfully")
                self.logger.info(f"Session name: {result.get('session_name', 'N/A')}")
            else:
                self.logger.error(f"Failed to start video transcoding client: {result['error']}")
                
            return result
            
        except Exception as e:
            self.logger.error(f"Exception during video transcoding client startup: {e}")
            return {
                'success': False,
                'error': str(e),
                'pid': None,
    ...
    def stop_video_transcoding_client(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Exception during video transcoding client cleanup: {e}")
            return {'success': False, 'error': str(e)}
    ...
    def start_video_transcoding_pmec_client(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['success']:
                self.logger.info("Video transcoding PMEC client started successfully")
                self.logger.info(f"Session name: {result.get('session_name', 'N/A')}")
            else:
                self.logger.error(f"Failed to start video transcoding PMEC client: {result['error']}")
                
            return result
            
        except Exception as e:
            self.logger.error(f"Exception during video transcoding PMEC client startup: {e}")
            return {
                'success': False,
                'error': str(e),
                'pid': None,
    ...
    def stop_video_transcoding_pmec_client(self) -> Dict[str, Any]:
        ...
        try:
            stop_cmd = (
                "tmux kill-session -t video_transcoding_pmec 2>/dev/null || true; "
                "sudo pkill -f 'streamer_subscriber' 2>/dev/null || true"
            )
            result = self.host_manager.execute_on_host(
                host_name="amari",
            ...
            result['success'] = True
            self.logger.info("Video transcoding PMEC client stopped successfully")
            return result
            
        except Exception as e:
            self.logger.error(f"Exception during video transcoding PMEC client cleanup: {e}")
            return {'success': False, 'error': str(e)}
    ...
    def get_client_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['success'] and result.get('output'):
                ...
                status['video_transcoding_pmec'] = 'video_transcoding_pmec:' in output
