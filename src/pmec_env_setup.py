...
class PMECEnvSetup:
    ...
    def restart_lte_service(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['success']:
                self.logger.info("LTE service restart command executed successfully")
                self.logger.info(f"Exit code: {result.get('exit_code', 'N/A')}")
                if result.get('output'):
                    self.logger.info(f"Output: {result['output']}")
            else:
                self.logger.error(f"Failed to restart LTE service: {result['error']}")
            return result
    ...
    def start_5g_gnb(self) -> Dict[str, Any]:
        ...
        gnb_command = (
            ...
            "sudo apps/gnb/gnb -c ../configs/gnb_rf_x310_tdd_n78_80mhz-63-samsung.yml "
    ...
    def start_pmec_controller(self) -> Dict[str, Any]:
        self.logger.info("Starting PMEC controller on edge0 (conda env pmec)...")
        ...
        try:
            ...
            if result['success']:
                ...
                self.logger.error(f"Failed to start PMEC controller: {result['error']}")
            return result
        except Exception as e:
    ...
    def setup_complete_environment(self, wait_time: int = 10) -> Dict[str, Any]:
        ...
        if not results['lte_restart']['success']:
            ...
            return results
        self.logger.info(f"Waiting {wait_time} seconds for LTE service to stabilize...")
        ...
        self.logger.info("Step 3: Starting PMEC controller on edge0 (conda env pmec)")
        ...
        results['overall_success'] = (
            results['lte_restart']['success'] and
            results['5g_gnb_start']['success'] and
            results['pmec_controller_start']['success']
        )
    ...
    def cleanup_environment(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info("PMEC controller cleanup completed (tmux session killed)")
        except Exception as e:
            self.logger.error(f"Exception during PMEC controller cleanup: {e}")
            results['pmec_controller_stop'] = {'success': False, 'error': str(e)}
        # Stop 5G gNB
        ...
        results['overall_success'] = (
            results['pmec_controller_stop']['success'] and
            results['5g_gnb_stop']['success'] and
            results['lte_stop']['success']
        )
        if results['overall_success']:
            self.logger.info("PMEC environment cleanup completed successfully!")
        else:
            self.logger.warning("PMEC environment cleanup completed with some errors")
        return results 