...
class PMECController:
    ...
    def start_pmec_server(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['success']:
                ...
                self.logger.error(f"Failed to start PMEC controller server: {result['error']}")
                
            return result
            
        except Exception as e:
            self.logger.error(f"Exception during PMEC controller server startup: {e}")
            return {
                'success': False,
                'error': str(e),
                'pid': None,
    ...
    def stop_pmec_server(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Exception during PMEC controller server cleanup: {e}")
    ...
    def start_pmec_client(self) -> Dict[str, Any]:
        ...
        command = (
            "cd ~/edge-client-prober && "
            "python run_amarisoft.py 2"
        )
        ...
        try:
            ...
            if result['success']:
                ...
                self.logger.error(f"Failed to start PMEC controller client: {result['error']}")
                
            return result
            
        except Exception as e:
            self.logger.error(f"Exception during PMEC controller client startup: {e}")
            return {
                'success': False,
                'error': str(e),
                'pid': None,
    ...
    def stop_pmec_client(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Exception during PMEC controller client cleanup: {e}")
            return {'success': False, 'error': str(e)}
    ...
    def get_pmec_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            server_result = self.host_manager.execute_on_host(
                host_name="edge1",
                command="tmux list-sessions 2>/dev/null | grep pmec_controller || echo 'No pmec_controller session'",
                background=False
            )
            ...
            if server_result['success'] and server_result.get('output'):
                status['server_running'] = 'pmec_controller:' in server_result['output']
                
            if client_result['success'] and client_result.get('output'):
                status['client_running'] = 'pmec_controller:' in client_result['output']
            
            # Overall system status
            status['system_running'] = status['server_running'] and status['client_running']
