...
def execute_command(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        logging.info(
            f"Executing command on {result['connection_info']}: {command}"
        )

        # Execute the command
        execution_result: Result = connection.run(command, hide=True, warn=True)
...
def execute_background_command(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        logging.info(
            f"Executing background command on {result['connection_info']}:"
            f" {command}"
        )
        ...
        if execution_result.exited == 0:
            ...
            if result["output"]:
                ...
                logging.info(
                    "Background process started in tmux session:"
                    f" {result['output']}"
                )
