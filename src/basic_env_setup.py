...
class BasicEnvSetup:
    ...
    def check_lte_service_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['success']:
                ...
                self.logger.error(f"Failed to check LTE service status: {result['error']}")
    ...
    def check_5g_gnb_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            tmux_result = self.host_manager.execute_on_host(
                host_name="edge0",
                command="tmux list-sessions 2>/dev/null | grep srsran || echo 'No srsran session'",
                background=False
            )
            ...
            process_result = self.host_manager.execute_on_host(
                host_name="edge0",
                command="ps aux | grep 'gnb.*configs' | grep -v grep || echo 'No gnb process'",
                background=False
            )
            ...
            if not result['output'].strip():
                self.logger.warning("5G gNB is not running (no tmux session or process found)")
    ...
    def setup_complete_environment(self, wait_time: int = 10) -> Dict[str, Any]:
        ...
        self.logger.info(f"Waiting {wait_time} seconds for 5G gNB to initialize...")
        time.sleep(wait_time)
    ...
    def cleanup_environment(self) -> Dict[str, Any]:
        ...
        if results['overall_success']:
            ...
            self.logger.warning("Environment cleanup completed with some errors")
