...
class TUTTIEnvSetup:
    """TUTTI environment setup for LTE, 5G, and TUTTI controller testing."""
    ...
    def start_tutti_controller(self) -> Dict[str, Any]:
        ...
        # tutti_command = (
        #     f"cd {srsran_path}/tutti_controller && "
        #     "~/miniconda3/bin/conda run -n tutti python3 tutti_controller.py"
        # )
        tutti_command = f"cd {srsran_path}/tutti_controller && python3 main.py"
        ...
        try:
            ...
            if result["success"]:
                self.logger.info("TUTTI controller started successfully")
                self.logger.info(
                ...
                self.logger.error(
                    f"Failed to start TUTTI controller: {result['error']}"
                )
            return result
        except Exception as e:
            self.logger.error(f"Exception during TUTTI controller startup: {e}")
            return {
                "success": False,
                "error": str(e),
                "pid": None,
    ...
    def setup_complete_environment(self, wait_time: int = 10) -> Dict[str, Any]:
        self.logger.info("Starting complete TUTTI environment setup...")
        results = {
            "lte_restart": None,
            "5g_gnb_start": None,
            "tutti_controller_start": None,
        ...
        if not results["tutti_controller_start"]["success"]:
            self.logger.error("TUTTI controller startup failed")
            return results
        results["overall_success"] = (
            results["lte_restart"]["success"]
            and results["5g_gnb_start"]["success"]
            and results["tutti_controller_start"]["success"]
        )
        if results["overall_success"]:
            self.logger.info("Complete TUTTI environment setup successful!")
        else:
            self.logger.error("TUTTI environment setup completed with errors")
        return results

    def cleanup_environment(self) -> Dict[str, Any]:
        self.logger.info("Starting TUTTI environment cleanup...")
        results = {
            "lte_stop": None,
            "5g_gnb_stop": None,
            "tutti_controller_stop": None,
            "overall_success": False,
        }
        ...
        try:
            ...
            self.logger.info(
                "TUTTI controller cleanup completed (tmux session killed)"
            )
        except Exception as e:
            self.logger.error(f"Exception during TUTTI controller cleanup: {e}")
            results["tutti_controller_stop"] = {
                "success": False,
                "error": str(e),
            }
        ...
        results["overall_success"] = (
            results["tutti_controller_stop"]["success"]
            and results["5g_gnb_stop"]["success"]
            and results["lte_stop"]["success"]
        )
        if results["overall_success"]:
            self.logger.info(
                "TUTTI environment cleanup completed successfully!"
            )
        else:
            self.logger.warning(
                "TUTTI environment cleanup completed with some errors"
            )
        return results
