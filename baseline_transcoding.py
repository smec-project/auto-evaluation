...
def start_experiment():
    ...
    if not basic_results.get('overall_success', False):
        ...
        logger.error(f"LTE restart success: {basic_results.get('lte_restart', {}).get('success', False)}")
        logger.error(f"5G gNB start success: {basic_results.get('5g_gnb_start', {}).get('success', False)}")
        return False
    
    logger.info("✓ Basic environment setup completed successfully")
    logger.info(f"  - LTE service restarted on amari")
    logger.info(f"  - 5G gNB started on edge0 (PID: {basic_results.get('5g_gnb_start', {}).get('pid', 'N/A')})")
...
def print_usage():
    ...
    print("    2. Start app servers on edge1 (file transfer + video transcoding)")
    print("    3. Start app clients on amari (file transfer + video transcoding)")
    print("")
    print("  End (1):")
